(ns advent-of-code-2020.day1.sum-2020)

(defn get-2020-sum-pair [list n]
  (loop [s (sort list)]
    (if (< (count s) 2)
      []
...
(defn get-2020-sum-tri-pair [list n]
  (loop [s (sort list)]
    (let [elem (first s)
          pair (get-2020-sum-pair (rest s) (- n elem))]
      (if (= 2 (count pair))
        (conj pair elem)
        (recur (rest s))))))

(defn get-2020-sum-tri-pair-brute-force [list n]
  (first
    (for [e1 list
          e2 list
          e3 list
